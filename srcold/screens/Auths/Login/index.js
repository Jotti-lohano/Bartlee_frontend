...
function Login() {
  ...
  const handleClick = async (e) => {
    ...
    let response = await axios.post("http://localhost/BARTLEE/bartlee/api/v1/user/login", formData).catch((e) => {
      console.log('response',e.response.data.error);
      if(e.response.data.email || e.response.data.password ){
        document.getElementById(
          "responseFromServer"
        ).innerHTML = `<div class="alert alert-danger"role="alert"><strong>Oops !</strong> ${e.response.data.email} and ${e.response.data.password}</div>`;
      }else{
        document.getElementById(
          "responseFromServer"
